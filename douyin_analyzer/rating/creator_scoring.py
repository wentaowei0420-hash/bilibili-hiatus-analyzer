...
class DouyinCreatorScorer:
    ...
    def _load_followed_liked_s_uids(self):
        ...
        if not active_uids or not self._table_exists("douyin_video_state", self.source_db_path):
            return set()
    ...
    def _load_creator_rows(self):
        main_rows = self._read_table("main_sheet_current", self.source_db_path)
        analysis_rows = self._read_table("analysis_sheet_current", self.source_db_path)
        inventory_rows = self._read_table("cache_inventory_current", self.source_db_path)
        creators = {}
    ...
    def _load_video_stats(self):
        rows = (
            self._read_table("video_score_current", self.rating_db_path)
            if self._table_exists("video_score_current", self.rating_db_path)
            else []
        )
        grouped = {}
        for row in rows:
    ...
    def _load_full_mode_uids(self):
        if not self._table_exists("cache_inventory_current", self.source_db_path):
            return self._load_full_mode_uids_from_progress()

        rows = self._read_table("cache_inventory_current", self.source_db_path)
        ...
        try:
            ...
            result -= load_active_archived_uids(self.source_db_path)
        except Exception:
            pass
        return result
    ...
    def _read_table(self, table_name, db_path=None):
    ...
    def _table_exists(self, table_name, db_path=None):
        db_path = Path(db_path or self.source_db_path)
